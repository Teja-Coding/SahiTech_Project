...
public class RequesterUnitGroupServiceImpl implements RequesterUnitGroupService {
	...
	public String getUnitGroupCode() {
		...
		Query query = (Query) session.createSQLQuery( "SELECT T_REQUESTER_GROUPS_SEQ.nextval from dual" );
		String SeqNo = ((org.hibernate.query.Query) query).list().iterator().next().toString();
