...
import my.com.cmg.iwp.maintenance.service.ReqCancelLogService;
import my.com.cmg.iwp.util.ApplicationConstants;
@Service
public class ReqCancelLogServiceImpl implements ReqCancelLogService {
	private BasisNextidDaoImpl<ReqCancelLog> reqCancelDAO;

	public BasisNextidDaoImpl<ReqCancelLog> getReqCancelDAO() {
		return reqCancelDAO;
	}

	public void setReqCancelDAO(BasisNextidDaoImpl<ReqCancelLog> reqCancelDAO) {
		this.reqCancelDAO = reqCancelDAO;
	}

	@Override
	public ResultObject getAllRequestLogs(int start, int pageSize) {
		DetachedCriteria criteria = DetachedCriteria
				.forClass(ReqCancelLog.class);
		criteria.addOrder(Order.asc("reqSeqNo"));
		int totalCount = getReqCancelDAO().findByCriteria(criteria).size();
		List<ReqCancelLog> list = getReqCancelDAO().findByCriteria(criteria,
				start, pageSize);
	...
	public List<ReqCancelLog> getAllRequestLogs() {
		DetachedCriteria criteria = DetachedCriteria
				.forClass(ReqCancelLog.class);
		criteria.addOrder(Order.asc("reqSeqNo"));
		return getReqCancelDAO().findByCriteria(criteria);
	}
	...
	public List<IntegrationBean> findByCriteria(String dateFrom,String dateTo,String ptjCode)  {
		/*String sql = "select count(createDate) as noOfTransactions, trunc(createDate) as "
				+ " integrationDate, status as status, poHdr.facilityCode from ReqCancelLog where 1 = 1 ";
		...
		String sql = "select extFac.facility_Name,data.noOfTransactions,data.integrationDate,data.status, poHdr.FACILITY_CODE " +
				" from ( select count(create_Date) as noOfTransactions, trunc(create_Date) as integrationDate, status as status, PO_HDR_SEQNO from" +
				" T_REQ_CANCEL_LOG where 1 = 1";
		...
		sql += " group by trunc(create_Date), status, PO_HDR_SEQNO  order by trunc(create_Date) desc) data " +
				"inner join t_po_hdr poHdr on poHdr.PO_HDR_SEQNO=data.PO_HDR_SEQNO inner join T_EXTERNAL_FACILITIES extFac on" +
				" extFac.facility_Code=poHdr.FACILITY_CODE where extFac.SERVER_IP IS NOT NULL  group by  poHdr.FACILITY_CODE,extFac.facility_Name,data.noOfTransactions,data.integrationDate,data.status " +
				" order by data.integrationDate desc";
		List<IntegrationBean> itemLogs = new ArrayList<IntegrationBean>();
		Session session = null;
		try {
			session = getReqCancelDAO().getSessionFactory().openSession();
			Query que = (Query) session.createSQLQuery(sql);
			List<Object[]> list = ((org.hibernate.query.Query) que).list();
			...
			for (int i = 0; i < list.size(); i++) {
				...
				log.setIntegrationType("Requisition Cancellation");
				log.setIntType(ApplicationConstants.REQC_CANC);
