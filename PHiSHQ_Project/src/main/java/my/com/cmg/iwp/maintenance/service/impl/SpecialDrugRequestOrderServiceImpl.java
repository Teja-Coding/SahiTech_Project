...
//		criteria.add(Restrictions.eq("sendFlag", RefCodeConstant.BOOLEAN_YES));
		...
		try {
		Query query =  (Query) session.createQuery("from SpecialDrugRequestOrder as sdr left join fetch sdr.specialDrugPatients as sdp left join fetch sdp.specialDrugTreatments sdt left join fetch sdr.specialDrugAdditionalDoc sdad left join fetch sdad.createdUser where sdr.sendFlag = 'Y'");
...
//		return getSpecialDrugRequestOrderDAO().findByCriteria(criteria);
	...
	public void createSDRTasklist(SpecialDrugRequestOrder specialDrugRequestOrder) {
		...
		try {
			...
			Query query = (Query) session.createSQLQuery("INSERT INTO t_task_lists(tl_seqno, tl_task_no, tl_doc_seqno, tl_rec_date_time, "
